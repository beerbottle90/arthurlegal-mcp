...
**Poland 🇵🇱 legal research over the Model Context Protocol.**
api.sejm.gov.pl/eli — ELI-compliant, no auth
...
The Sejm API is good but searches **titles only**. This server adds a local index over titles plus the Sejm's own controlled subject keywords, and surfaces the `references` amendment graph that Polish research turns on.
...
For cross-language work — a Turkish question against a Poland corpus — the
...
- Search matches titles only, never act bodies.
- Many acts are PDF-only (`textHTML: false`) — the server checks before fetching HTML.
- Responses are gzip-encoded; urllib does not decompress automatically.
...
```sh
docker build -t pl-sejm-mcp .
docker run -p 8000:8000 -e PORT=8000 pl-sejm-mcp
```
...
| Variable | Effect |
|---|---|
| `CRAWL_ARGS` | What to index on first boot (default `--from 2015 --to 2026`) |
